...
- poll 端另有兜底：持鎖超過 `max-time + 60 s` ⇒ `SIGKILL`。兩者的期限都取自 `meta.json`（worker 在拿鎖後讀一次 `max_time`；poll 每次呼叫讀 `started_at`／`max_time`／`grace`）——沒有另一個 deadline 檔，但 `meta.json` 本身寫得到 base 者就能改寫（§6 第 (5) 項）。本 bullet 保證的只有**損毀**：meta 不可解析，或 `started_at`／`max_time` 缺失或型別錯時，poll **fail-closed**（視為逾時，kill + TIMEOUT），不會退化成無限 RUNNING（`grace` 型別錯則退回 60 s）。值合法但由攻擊者選定的期限**不在此保證內**，§6 第 (5) 項的期限面列了它的三種結果：提早 kill、推遠期限而回 `RUNNING`、以及型別合法的極端值讓 poll（與 worker）溢位當掉、stdout 沒有 token。
...
- 硬化（§3）與 lock 完整性（§4）**不建立**針對「寫得到 base」的安全邊界。它們買到的是消掉最便宜的兩種 lock 檔變體與一行誠實。**寫得到 base 者可以，封閉列舉六項：** (1) 殺掉本工具的 worker（直接 kill——同 uid 本來就能）；注意把 victim 持鎖的檔案 `rename` 進 `<run>/lock` 的效果是**相反**的：`--abort` 對 `F_GETLK` 回報的 pid 送訊號，殺的是 **victim**、worker 反而活成孤兒、run 被刪、印 `ABORTED` exit 0（round 10 實測）——那落在第 (6) 項，不是這裡（round 10 把它寫在 (1) 底下，歸因反了）；(2) 刪掉 base 下的任何 run 與預設輸出；(3) `touch <run>/reported`——讓一個已付費的成功結果被下一個 caller 只清不報，caller 得到 exit 1（round 7 Q3）；(4) 把 `<run>/claim` 換成 symlink／FIFO／目錄／hard link 或直接 `rm`——讓 claim `.failed`（run 原地保留、需人工清理），**或**讓 GC 誤判（round 9 A2 之後 GC 對不可判定一律不刪，所以這一項的傷害從「掃掉正在被 finalize 的 run」降為「留下需人工清理的殘留」）；(5) 改寫 `meta.json` 的**任何欄位**或 `prompt.txt`——這兩個檔**沒有完整性驗證，內容只驗型別、不驗值**：§4 的 `O_NOFOLLOW`＋`fstat` 只套在 `lock`／`claim`；`meta.json` 只要能解析成 JSON 物件就收（`readMeta`），欄位只看型別、不查範圍，型別不符的欄位多半退回預設值；`prompt.txt` 以一般路徑讀取（symlink 照跟）。本工具從 meta 讀的欄位共十三個——`output`、`default_output`、`max_time`、`started_at`、`grace`、`model`、`effort`、`service_tier`、`instructions`、`selftest_prelock`、`selftest_sleep`、`selftest_fail`、`selftest_ignore_term`（這個數字是對 `bin/codex-call` 全部 meta 讀取點的**手動清點，沒有 lint 守護**；改 code 時以讀取點為準重數）——逐欄的效果如下（#54 對 `bin/codex-call` 逐行核對；標「讀碼推得」者未實際執行）。**時序**——欄位分兩類。worker 端的欄位是快照：worker 在拿鎖**之前**讀一次 meta（只取 `selftest_prelock`）、拿鎖**之後**再讀一次（其餘欄位），之後不再讀；不走 `selftest_sleep` 路徑時再讀 `prompt.txt` 一次。所以送出面、回流、寫入面、測試鉤子面與 worker 自己的 `max_time`，要在 `--detach` 寫完這兩個檔之後、worker 讀到之前改寫（這個視窗含 swift 啟動；`doDetach` 註解實測 spawn 後 0.6–1.0 s 才拿到鎖）：對只能寫檔的跨 uid 寫入者，這是競態、不是不可達；**同 uid** 者則有確定性的路徑——例如在 worker 讀 meta 之前對它送 `SIGSTOP`、改寫後再 `SIGCONT`（同 uid 本來就能對你的任何程序送訊號，見本節第一列；讀碼推得）。poll 端的欄位——期限用的 `started_at`／`max_time`／`grace`、回報用的 `output`、清理用的 `default_output`——`--poll` 每次呼叫都重讀，run 存在的整段期間都可達（`--abort` 也讀一次，只用 `default_output`）。**送出面**——`prompt.txt`＋`instructions`＋`model`／`effort`／`service_tier`（未知的 tier 值原樣送出）讓 worker **用受害者的 ChatGPT OAuth 送出攻擊者寫的 prompt 與 system instructions**，即**配額竊取＋任意生成**；`max_time` 同時是 worker 的 URLSession timeout 與 semaphore 兜底，所以單一請求能燒多久也由攻擊者決定；`prompt.txt` 換成 symlink，worker 就把受害者讀得到的那個檔的內容送給 Codex。**回流**——送出面只是一半：同一個攻擊者也控制 `instructions`（例如「逐字複述輸入」）與 `output`，而 worker 用 `write(toFile:atomically:)` 寫出的檔在 umask 022 下是 `0644`（security lens 實測），所以受害者讀得到的任意檔**可以被讀回來**——`prompt.txt` 指向它、`output` 指向攻擊者讀得到的位置。最直接的目標是 `~/.codex/auth.json`：`AUTH_FILE` 取自 `homeDirectoryForCurrentUser`（security lens 實測不隨 `$HOME`），而 base（`runsBase()`）取自 `$HOME`——在 #54 引用的 HOME 注入情境下，base 被攻擊者影響、憑證仍在受害者真正的家目錄。這是**憑證外洩**（`auth.json` 含 refresh token），不只是配額竊取；它是否發生**取決於模型是否照 instructions 逐字複述**，本工具沒有任何一層阻止或偵測它。**寫入面**——`output` 是 worker 寫結果的路徑：一般路徑寫 Codex 的回應，`selftest_sleep` 路徑不發 HTTP、寫固定內容 `SELFTEST`；兩者都以受害者身分經 Foundation 的 `write(toFile:atomically: true)`，也就是先寫暫存檔再 rename 到該路徑——結果是在攻擊者選定的路徑放上一個新檔，而不是原地寫進既有的檔。實際寫得到哪裡取決於受害者對該路徑所在目錄的權限與 Foundation 的暫存檔處理，**不等於**「受害者有權寫入的任意檔」（DA 實測兩個方向都有例外；本契約不列舉其邊界）。內容：一般路徑由送出面的 prompt／instructions 左右（取決於模型），`SELFTEST` 路徑則是**確定性的**固定內容覆寫。**回報面**——`--poll` 印 `DONE <output>` 的條件只有兩個：`<run>/status` 去掉空白後等於 `0`，且對 `output` 取 `attributesOfItem` 得到的 size 大於 0。它不要求那個檔是 worker 寫的；`attributesOfItem` 不跟 symlink（Foundation 語意，讀碼推得），量到的是 symlink 自己——**懸空 symlink 也會讓 poll 印 `DONE`**。caller 會去讀那個路徑（round 8 DA §4.1 實測）；連同送出面，攻擊者自撰的文字會被當成「Codex 的審閱」交給 wrapper agent（round 10 S10-4，#54）。**期限面**——`started_at`／`max_time`／`grace` 是 poll 逾時兜底的全部輸入，只在 worker 持鎖時用到，結果有三種：（a）值合法但選成已過期 → poll 對活的 worker 提早 claim、kill 並印 `TIMEOUT`；（b）把期限推遠 → poll 回 `RUNNING`，但 worker 拿鎖時讀到的 `max_time` 仍會終止它，所以要「一直」回 `RUNNING`，必須同時在上面的時序視窗內改寫 worker 端的 `max_time`，或加上 `selftest_sleep`（該路徑完全不經 `max_time`）；（c）**型別合法的極端值讓程序當掉**：poll 的期限判斷用 Swift 的 checked `Int` 算術，`max_time`／`grace` 接近 `Int.max` 或 `started_at = Int.min` 會溢位 trap（logic lens 以本機 swift 實測 exit 133）——worker 持鎖期間每次 `--poll`（含 `--wait`）都當掉、stdout 沒有任何 token，直到 worker 自己結束（之後 poll 走未持鎖分支、不算期限）；這段期間的出路是 `--abort`，它不算期限。worker 端 `max_time` 接近 `Int.max` 時，semaphore 兜底的 `max-time + 5` 同樣溢位 trap（讀碼推得；發生在請求已送出之後），worker 死而無 status → poll 印 `FAILED status missing`。§5 的 fail-closed 只擋「meta 不可解析，或 `started_at`／`max_time` 缺失或型別錯」，不擋這三種。**測試鉤子面**——四個 `selftest_*` 在 worker 端是從 meta 讀、不是從旗標讀（旗標只決定 `--detach` 寫不寫進 meta），時序同 worker 端欄位：`selftest_prelock`（拿鎖前讀，只在值為正整數時生效）讓 worker 拿鎖前睡該秒數，超過 `--detach` 的 20 s readiness 視窗時 detach 終止 worker、清掉 run 並失敗，值超過 `UInt32` 上限則 trap（讀碼推得）；`selftest_sleep`（只要是整數就生效，負值當 0）讓一個正式 run 不發 HTTP、睡該秒數（期間不受 `max_time` 約束）、再把 `SELFTEST` 寫進 `output` 並寫 status `0` → poll 印 `DONE`，值超過 `UInt32` 上限同樣 trap（讀碼推得），另外讓該 run 落入 `--_selftest-gc-age` 的覆寫範圍（任何帶該旗標的 `--detach` 的 GC 都可能掃掉它，效果不超出第 2 項；這一點不受上面的時序限制——GC 每次掃描都重讀 meta）；`selftest_fail` **只在 `selftest_sleep` 生效時**才讀，睡完寫 status `9 selftest failure`、不寫 `output` → poll 印 `FAILED 9 selftest failure`，把正式 run 換成一個不發 HTTP 的失敗；`selftest_ignore_term` **只在 `selftest_sleep` 生效時**才讀，讓 worker 對 SIGTERM 裝 handler（touch `<run>/term-seen`）並睡回剩餘秒數——poll／abort 在 SIGTERM 後 2 s 升級成 SIGKILL，所以它只延後終止、不讓 worker 逃過終止。**`default_output`**——只決定可推導的 `<base>/<id>.out.md` 在非 `DONE` 終態刪不刪（只 unlink 一般檔），刪除路徑從不取自 meta（round 5 S2），效果落在第 2 項之內。**這一項直接餵進本節末的 prompt-injection 鏈**，是「刪除面」（round 5 S2 已修）之外的「送出面」「回流」「寫入面」「回報面」「期限面」與「測試鉤子面」，同一句上界曾因此兩次為假（#54 是第三次補齊）；(6) **讓工具對一個活著的 worker 印出終態並刪掉 run，或讓 `--abort` 永久無法止血**——`rm <run>/lock`、換成全新一般檔、或把 victim 持鎖的檔案 `rename` 進來（前兩者四項完整性檢查全過，第三者三檢查全過）→ `--poll` 印 `FAILED status missing` 並刪 run、`--abort` 印 `ABORTED` exit 0，worker 續跑成永久孤兒（S9-1）；`mkfifo <run>/lock` → `--abort` 在殺 worker 之前被拒：**stdout 無 token、stderr 一行 `cannot be trusted or inspected`**（S9-2；round 10 曾寫「無任何 stderr 行」，為假）。round 9 security 指出這既不是第 1 項（殺自己的 worker）也不是第 2 項（刪自己的 run），是第六類傷害；逃生命令 `--force-reap <id>`（§2）。**以上六項為封閉列舉，不得依性質相似類推第七項。** 本工具**不宣稱防禦**其中任何一項；列出它們是為了讓 caller 知道「同 uid 在模型外」這句話具體換來的是什麼。
...
- `--_selftest-gc-age N`：與 `--_selftest-sleep` 並用，覆寫 24 h 的 GC 門檻——**只對 meta 含 `selftest_sleep` 的 run 生效**（round 6 RC5），沒帶 `--_selftest-sleep` 啟動的正式 run 不受影響——前提是它的 meta 沒被改寫：`selftest_sleep` 是從 meta 讀的，寫得到 base 者在正式 run 的 meta 加上它，該 run 就落入覆寫範圍（§6 第 (5) 項的測試鉤子面）。
