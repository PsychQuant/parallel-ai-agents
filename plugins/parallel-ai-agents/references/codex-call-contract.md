...
- poll 端另有兜底：持鎖超過 `max-time + 60 s` ⇒ `SIGKILL`。兩者都不依賴任何可被竄改的 deadline 檔——期限資訊寫在 `meta.json`，但 meta 損毀時 poll **fail-closed**（視為逾時，kill + TIMEOUT），不會退化成無限 RUNNING。這只涵蓋「損毀」（欄位缺失或型別錯）；`meta.json` 本身可被寫得到 base 者改寫，值合法但由攻擊者選定的期限不在此保證內（§6 第 (5) 項的期限面）。
...
- 硬化（§3）與 lock 完整性（§4）**不建立**針對「寫得到 base」的安全邊界。它們買到的是消掉最便宜的兩種 lock 檔變體與一行誠實。**寫得到 base 者可以，封閉列舉六項：** (1) 殺掉本工具的 worker（直接 kill——同 uid 本來就能）；注意把 victim 持鎖的檔案 `rename` 進 `<run>/lock` 的效果是**相反**的：`--abort` 對 `F_GETLK` 回報的 pid 送訊號，殺的是 **victim**、worker 反而活成孤兒、run 被刪、印 `ABORTED` exit 0（round 10 實測）——那落在第 (6) 項，不是這裡（round 10 把它寫在 (1) 底下，歸因反了）；(2) 刪掉 base 下的任何 run 與預設輸出；(3) `touch <run>/reported`——讓一個已付費的成功結果被下一個 caller 只清不報，caller 得到 exit 1（round 7 Q3）；(4) 把 `<run>/claim` 換成 symlink／FIFO／目錄／hard link 或直接 `rm`——讓 claim `.failed`（run 原地保留、需人工清理），**或**讓 GC 誤判（round 9 A2 之後 GC 對不可判定一律不刪，所以這一項的傷害從「掃掉正在被 finalize 的 run」降為「留下需人工清理的殘留」）；(5) 改寫 `meta.json` 的**任何欄位**或 `prompt.txt`——這兩個檔**沒有任何完整性或內容驗證**：§4 的 `O_NOFOLLOW`＋`fstat` 只套在 `lock`／`claim`，`meta.json` 只要能解析成 JSON 物件就收、欄位只看型別不看值，`prompt.txt` 以一般路徑讀取（symlink 照跟）。本工具從 meta 讀的欄位共十三個，逐欄的效果如下（#54 對 `bin/codex-call` 逐行核對）：**送出面**——`prompt.txt`＋`instructions`＋`model`／`effort`／`service_tier`（未知的 tier 值原樣送出）讓 worker **用受害者的 ChatGPT OAuth 送出攻擊者寫的 prompt 與 system instructions**，即**配額竊取＋任意生成**；`max_time` 同時是 worker 的 URLSession timeout 與 semaphore 兜底，所以單一請求能燒多久也由攻擊者決定；`prompt.txt` 換成 symlink，worker 就把受害者讀得到的那個檔的內容送給 Codex。時序：worker 在拿到 lock **之後**才讀 `meta.json`（`selftest_prelock` 例外，拿鎖前讀）、再讀 `prompt.txt`，各一次，所以送出面與寫入面要在 `--detach` 寫完這兩個檔之後、worker 讀到之前改寫（這個視窗含 swift 啟動；`doDetach` 註解實測 spawn 後 0.6–1.0 s 才拿到鎖）——是競態，不是不可達；回報面與期限面則不受此限，`--poll` 每次呼叫都重讀 meta，run 活著的整段期間都可達。**寫入面**——`output` 是 worker 寫 Codex 回應的路徑（以受害者身分、`write(toFile:atomically:)` 暫存檔＋rename，取代該路徑上原有的檔），也就是對受害者有權寫入之任意路徑的覆寫，內容由上一句的 prompt 左右。**回報面**——`output` 讓 `--poll` 印出 `DONE <攻擊者選定路徑>`（條件只有 status 為 `0` 且該路徑非空，不要求那個檔是 worker 寫的），caller 會去讀那個檔（round 8 DA §4.1 實測）；連同送出面，攻擊者自撰的文字會被當成「Codex 的審閱」交給 wrapper agent（round 10 S10-4，#54）。**期限面**——`started_at`／`max_time`／`grace` 是 poll 逾時兜底的全部輸入：值合法但由攻擊者選定時，poll 可以對一個活的 worker 提早 kill 並印 `TIMEOUT`，也可以把期限推到很遠而一直回 `RUNNING`（§5 的 fail-closed 只擋欄位缺失或型別錯）。**測試鉤子面**——`selftest_prelock`／`selftest_sleep`／`selftest_fail`／`selftest_ignore_term`（§7）在 worker 端是從 meta 讀、不是從旗標讀：`selftest_sleep` 讓一個正式 run 不發 HTTP、把 `SELFTEST` 寫進 `output` 並回 `DONE`，`selftest_ignore_term` 讓 worker 活過 SIGTERM，`selftest_prelock` 讓 worker 拿鎖前睡任意秒數（超過 `--detach` 20 s 的 readiness 視窗，detach 就終止 worker 並失敗）；`selftest_sleep` 另外讓該 run 落入 `--_selftest-gc-age` 的覆寫範圍，效果不超出第 2 項。唯一不擴大傷害的欄位是 `default_output`：它只決定可推導的 `<base>/<id>.out.md` 刪不刪，刪除路徑從不取自 meta（round 5 S2）；**這一項直接餵進本節末的 prompt-injection 鏈**，是「刪除面」（round 5 S2 已修）之外的「回報面」「送出面」「寫入面」「期限面」與「測試鉤子面」，同一句上界曾因此兩次為假（#54 是第三次補齊）；(6) **讓工具對一個活著的 worker 印出終態並刪掉 run，或讓 `--abort` 永久無法止血**——`rm <run>/lock`、換成全新一般檔、或把 victim 持鎖的檔案 `rename` 進來（前兩者四項完整性檢查全過，第三者三檢查全過）→ `--poll` 印 `FAILED status missing` 並刪 run、`--abort` 印 `ABORTED` exit 0，worker 續跑成永久孤兒（S9-1）；`mkfifo <run>/lock` → `--abort` 在殺 worker 之前被拒：**stdout 無 token、stderr 一行 `cannot be trusted or inspected`**（S9-2；round 10 曾寫「無任何 stderr 行」，為假）。round 9 security 指出這既不是第 1 項（殺自己的 worker）也不是第 2 項（刪自己的 run），是第六類傷害；逃生命令 `--force-reap <id>`（§2）。**以上六項為封閉列舉，不得依性質相似類推第七項。** 本工具**不宣稱防禦**其中任何一項；列出它們是為了讓 caller 知道「同 uid 在模型外」這句話具體換來的是什麼。
