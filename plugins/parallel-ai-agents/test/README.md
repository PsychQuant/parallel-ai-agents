...
| `pai-codex-bundle.bats` | `../bin/pai-codex-bundle`（Codex leg 的 artifact 組裝器：檔案直通 `--prompt-file`、目錄組成有上限的 bundle；#45）|
...
`pai-codex-bundle.bats` 涵蓋：單一檔案逐 byte 直通（#37 路徑不變）、目錄 → bundle 且 `<cmd>` 返回後刪暫存檔、stdout 只屬於 `<cmd>`、exit code 照傳；排除規則（build 目錄、二進位、非 UTF-8、疑似祕密檔名、symlink no-follow、`.gitignore`）；位元組序；單檔／總量／檔案數（`--max-files`）上限與 `PAI-BUNDLE-TRUNCATED` 標注、截斷不切壞 UTF-8；隨機邊界 token 與換行檔名防偽造；空目錄／不存在／非法上限 → exit 1 且不呼叫 `<cmd>`。
