...
| `lint-ci-log-filter.sh` | 護欄：`.github/workflows/test.yml` 每一個 `run:` step 都必須經 `../pai-lenses/scripts/neutralise.py` 或帶 `# LOG-FILTER:` 註解明示不過濾與理由（#33 verify R15：散文列舉在第一輪就不封閉）。`--selftest` 對 `fixtures/ci-log-filter-bad.yml` 與四個 `ci-log-filter-bypass-*.yml`（`- run:` 無 name、flow mapping、註解裡的 neutralise.py、前導註解歸屬）拒絕、`ci-log-filter-good.yml` 接受；非 monorepo 佈局（無 .github/）明說略過 |
