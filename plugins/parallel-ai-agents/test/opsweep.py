...
EXPECTED_SURVIVE = {   # id → 理由（依構造等價）。每一條都要能回答「為什麼關掉它沒有任何輸出會變」
    ...
    # 逐行相符」這句話 R37 收窄成實話**（#33 verify R36 requirements 第 20 列）：逐行相符指的是
    # `shell_scan` 唯一在乎的**內容承載行**（有實際字元的行；blank-line 的計數／位置本身另有一個
    # 既有、與本節無關的簡化，見下方 R37 補的那段），且**縮排只算空白、不含 tab**——含 tab 的
    # dedent 判準是 item 2 修的（`dedent_block` 那支）。R37 用窮舉對帳（3 行 body，每行取 21 種
    # 形狀：leading `""`／`" "`／`"  "`／`"\t"`／`"\t\t"`／`" \t"`／`"\t "` 交叉「純空白／內容
    # `x`／內容 `x ` 帶行尾空白」，5,838 組合法 YAML）量到：修 item 2＋3 之前，literal 402 組、
    # folded 516 組不符；修完之後**兩種模式 0 組不符**。指令碼：`.r37e-scratch/exhaustive_check.py`
    # （工作包產物，未進 repo；方法留此供之後重跑）。
