...
- exit 1（不發訊號、不刪東西）：`invalid run id`（格式錯）；`unknown run id`（**沒有目錄且** `ps` 文字裡沒有程序帶此 id——S9-1 的孤兒只靠後者找到，所以「有程序沒目錄」不是 unknown）；`cannot list processes`（無身分來源，見上）。與 `--detach`／`--poll`／`--abort` 互斥（同時給 → exit 1）。
