...
MUTATIONS += [
    ...
    ("lint: block scalar 共同縮排先剝除（R26 M2）",
     ...
     "        run_code, shell_decls, run_unparsed = shell_scan(fold_block(run_lines, block_folded))", "lint"),
    ...
    ("lint: 折疊 block scalar 先折再掃（R30 H-5 → bypass-folded-scalar-comment）",
     "        run_code, shell_decls, run_unparsed = shell_scan(fold_block(dedent_block(run_lines, explicit_pad), block_folded))",
     "        run_code, shell_decls, run_unparsed = shell_scan(dedent_block(run_lines, explicit_pad))", "lint"),
...
MUTATIONS += [
    ...
    ("lint: --strict：接 neutralise 的管線要帶 2>&1／|&（R34 requirements F4 → bypass-strict-missing-2to1）",
     '        elif STRICT and not declared and (sum(len(PIPED_RE.findall(l)) for l in logical)', '        elif False and (sum(len(PIPED_RE.findall(l)) for l in logical)', "lint"),
